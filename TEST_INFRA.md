...
- **Gameplay fixture and benchmark**: `AZAHAR_CAPTURE_MANUAL=1 node tests/capture_gameplay_state.cjs` opens a visible UI. The tester clears kiosk menus, moves Mario in W1-1, and explicitly saves from an overlay/F8. `tests/benchmark_browser.cjs --state <fixture.cst>` installs that ignored local state in the normal web user-state directory, calls the non-blocking core load signal, and requires a non-black/colorful restored canvas before sampling. This is the benchmark mode for optimization work; boot/title-only runs are diagnostic.
- **Gameplay compositor/movement gate**: `node tests/browser_gameplay_state.cjs --state <fixture.cst>` restores W1-1 through the normal UI, captures the browser compositor (not canvas backing storage), holds circle-pad right, and requires another high-information visible frame with changed pixels. It is the mandatory renderer gate once a local moving fixture exists.
- **Accelerated-renderer preflight**: `tests/webgl2_preflight.cjs` probes WebGL2/WebGPU on a separate canvas and records browser limits/texture extensions without changing the production software canvas. It is a readiness report, not proof that a renderer exists; the mandatory backend contract is in `WEBGL2_RENDERER_CONTRACT.md`.
