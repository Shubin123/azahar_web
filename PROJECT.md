...
Artifacts are generated under `build-web/bin/Release/` and copied into `web/` for serving. `node tests/web_artifact_smoke.cjs` validates both artifact copies, the generated API names, and WASM compilation. `tests/browser_regression.cjs`, supplied with a decrypted local `.3ds` through `AZAHAR_ROM_PATH`, verifies cross-origin isolation, initialization, ROM mounting/loading, a first frame, the continuous run loop, canvas output, and browser errors. With `AZAHAR_REAL_ROM_BOOT_MS=45000`, the verified kiosk demo reached frame 1,349 and rendered its visible title framebuffer (162,999 non-black pixels; 942 sampled colors) without browser errors. An encrypted CIA is expected to fail cleanly with the UI's encrypted-ROM status. The legacy Node mock suite remains diagnostic only and needs modernization before it can gate the current Emscripten artifact.
